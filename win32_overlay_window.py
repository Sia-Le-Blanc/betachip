...
class Win32OverlayWindow:
    ...
    def __init__(self):
        ...
        # 모자이크 빠른 테스트를 위한 빨간색 사각형 생성 저장
        test_rect = np.ones((100, 100, 3), dtype=np.uint8)
        test_rect[:] = [0, 0, 255]  # BGR - 빨간색
        cv2.rectangle(test_rect, (0, 0), (99, 99), (255, 255, 255), 3)  # 흰색 테두리
        cv2.putText(test_rect, "TEST", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    def _create_window(self):
        ...
        try:
            ...
            # 윈도우 생성
            # 클릭 통과를 위한 WS_EX_TRANSPARENT 유지
            ...
            # 투명도 설정 - 150으로 설정하여 더 잘 보이게 함
            windll.user32.SetLayeredWindowAttributes(self.hwnd, 0, 150, 2)  # LWA_ALPHA = 2
    ...
    def _render_overlay(self):
        ...
        try:
            ...
            # 테스트용 사각형 그리기 - 완전히 빨간색, 더 커서 더 잘 보임
            red_brush = windll.gdi32.CreateSolidBrush(0x0000FF)  # BGR 색상 (빨간색)
            windll.gdi32.SelectObject(mem_dc, red_brush)
            
            # 화면 전체에 큰 사각형 그리기
            windll.gdi32.Rectangle(mem_dc, 100, 100, self.width-100, self.height-100)
            
            # 모서리에 작은 사각형 4개 그리기
            windll.gdi32.Rectangle(mem_dc, 0, 0, 200, 200)  # 왼쪽 상단
            windll.gdi32.Rectangle(mem_dc, self.width-200, 0, self.width, 200)  # 오른쪽 상단
            windll.gdi32.Rectangle(mem_dc, 0, self.height-200, 200, self.height)  # 왼쪽 하단
            windll.gdi32.Rectangle(mem_dc, self.width-200, self.height-200, self.width, self.height)  # 오른쪽 하단
            ...
            if self.mosaic_regions:
                ...
                # 모자이크 영역이 없으면 테스트 사각형 표시
                # 테스트 이미지 그리기 (중앙에 큰 빨간색 사각형)
                test_size = 300
                test_x = (self.width - test_size) // 2
                test_y = (self.height - test_size) // 2
                
                # 테스트 이미지 준비
                bgra_test = cv2.cvtColor(self.test_rect, cv2.COLOR_BGR2BGRA)
                bgra_test[:, :, 3] = 255  # 알파 채널 설정
                
                # 비트맵 헤더 준비
                bmi_test = BITMAPINFOHEADER()
                bmi_test.biSize = ctypes.sizeof(BITMAPINFOHEADER)
                bmi_test.biWidth = self.test_rect.shape[1]
                bmi_test.biHeight = -self.test_rect.shape[0]  # 상하 반전
                bmi_test.biPlanes = 1
                bmi_test.biBitCount = 32
                bmi_test.biCompression = 0  # BI_RGB
                
                # 이미지 그리기
                data_ptr_test = bgra_test.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte))
                windll.gdi32.SetDIBitsToDevice(
                    mem_dc, test_x, test_y, self.test_rect.shape[1], self.test_rect.shape[0],
                    0, 0, 0, self.test_rect.shape[0],
                    data_ptr_test,
                    byref(bmi_test),
                    0  # DIB_RGB_COLORS
                )
            ...
            bf.BlendFlags = 0
            bf.SourceConstantAlpha = 175  # 투명도 조정 (0-255, 값이 낮을수록 더 투명)
            ...
            UpdateLayeredWindow.argtypes = [
                ...
                ctypes.c_uint, ctypes.POINTER(BLENDFUNCTION), ctypes.c_uint
            ]
    ...
    def update_regions(self, original_image, mosaic_regions):
        ...
        try:
            ...
            # 완전히 빨간색으로 채운 모자이크 이미지로 변경
            enhanced_regions = []
            for x, y, w, h, label, mosaic_img in mosaic_regions:
                if mosaic_img is not None:
                    # 빨간색으로 채운 이미지 생성
                    red_img = np.zeros((h, w, 3), dtype=np.uint8)
                    red_img[:, :] = [0, 0, 255]  # BGR 형식 - 빨간색
                    
                    # 테두리와 텍스트 추가
                    cv2.rectangle(red_img, (0, 0), (w-1, h-1), (255, 255, 255), 2)  # 흰색 테두리
                    cv2.putText(red_img, label, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    
                    enhanced_regions.append((x, y, w, h, label, red_img))
                else:
            ...
            if len(mosaic_regions) > 0:
                if self.frame_count % 30 == 0:  # 더 자주 출력
    ...
    def _save_debug_image(self):
        ...
        try:
            ...
            for x, y, w, h, label, _ in self.mosaic_regions:
                # 원본 이미지에 박스 표시
                cv2.rectangle(debug_image, (x, y), (x+w, y+h), (0, 0, 255), 2)  # 빨간색으로 변경
